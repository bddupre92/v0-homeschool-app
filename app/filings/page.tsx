    ...
    <Shell>
      ...
      <section className="mt-12 pt-8 border-t border-[var(--rule)]">
        ...
        <p className="text-xs text-[var(--ink-4)] mt-4">
          Six filings shipped across four states (NY, PA, MA, OR). Additional states by demand.
        </p>
      </section>
    </Shell>
  )
...
function FilingsByYear({ filings }: { filings: any[] }) {
  // Group by school year (newest first), preserve newest-first order within
  // each year.
  const years = Array.from(new Set(filings.map((f) => f.school_year || "—"))).sort((a, b) =>
    b.localeCompare(a),
  )
  return (
    <div className="space-y-8">
      {years.map((year) => {
        const rows = filings.filter((f) => (f.school_year || "—") === year)
        return (
          <section key={year}>
            <h2 className="atoz-eyebrow mb-3">{year}</h2>
